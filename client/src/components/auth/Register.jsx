    ...
    useEffect(() => {
        if (auth.user || auth.userProfile) {
            if(notify.success){
                ...
                alert.error('Please Logout first.')
            }
            navigate('/')
        }
        if (auth.user) {
                            ...
                            <div className="card border-0 shadow rounded-3 my-5">
                                <div className="card-body p-4 p-sm-5">
                                    ...
                                    <form onSubmit={handleSubmit}>
                                        ...
                                        <hr className="my-4" />
                                        <div className="d-grid ">
                                        <button className="btn btn-google btn-login text-uppercase fw-bold" onClick={()=>{navigate('/login')}}>
                                                <i className="fab fa-google me-2"></i> Signin
                                            </button>
                                        </div>
                                    </form>
                                </div>
